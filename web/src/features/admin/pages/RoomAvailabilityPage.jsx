...
function RoomAvailabilityPage() {
  ...
  const { data: snapshot, isLoading: loading } = useDigitalTwinSnapshot(defaultBranch);
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      ...
      const matchesStatus = roomStatusFilter === "all" || (() => {
        ...
        const roomLevelMaintenance = bedsInMaintenance === room.capacity && room.capacity > 0;
        const effectiveCapacity = roomLevelMaintenance ? 0 : room.capacity - bedsInMaintenance;
        
        let displayStatus = 'available';
        if (roomLevelMaintenance) displayStatus = 'maintenance';
        else if (room.currentOccupancy >= effectiveCapacity && effectiveCapacity > 0) displayStatus = 'full';
        else if (room.currentOccupancy > 0) displayStatus = 'partial';
        return displayStatus === roomStatusFilter;
      })();

      return matchesSearch && matchesBranch && matchesFloor && matchesType && matchesStatus;
    });
  }, [rooms, searchTerm, branchFilter, floorFilter, roomTypeFilter, roomStatusFilter]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, branchFilter, floorFilter, roomTypeFilter, roomStatusFilter, forecastStatusFilter, activeTab]);
  ...
  const roomFilters = [
    ...
    {
      key: "branch",
      options: [{ value: "all", label: "All Branches" }, ...OWNER_BRANCH_FILTER_OPTIONS.filter(o => o.value !== "all")],
      value: branchFilter,
  ...
  const getRoomStatusConfig = (status) => {
    switch (status) {
      case 'available':
        return { dot: 'bg-green-500', label: 'Available', color: 'text-green-600' };
      case 'partial':
        return { dot: 'bg-amber-500', label: 'Partially Occupied', color: 'text-amber-600' };
      case 'full':
        return { dot: 'bg-red-500', label: 'Full', color: 'text-red-600' };
      case 'maintenance':
        return { dot: 'bg-neutral-500', label: 'Maintenance', color: 'text-neutral-600' };
      case 'reserved':
        return { dot: 'bg-blue-500', label: 'Reserved', color: 'text-blue-600' };
      default:
        return { dot: 'bg-border', label: 'Unknown', color: 'text-muted-foreground' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-foreground mb-1">Room Management</h1>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            ...
            <div className="bg-card border border-border rounded-lg p-3">
              ...
              <div className="text-2xl font-semibold text-foreground">{stats.available}</div>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-2xl font-semibold text-foreground">{stats.partial}</div>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-2xl font-semibold text-foreground">{stats.full}</div>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-2xl font-semibold text-foreground">{stats.maintenance}</div>
            </div>
            <div className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
              ...
              <div className="text-2xl font-semibold text-foreground">{stats.rate}%</div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex flex-col lg:flex-row gap-3 mb-6">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-muted-foreground/70"
                />
              </div>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {can("manageRooms") && (
                  <button 
                    ...
                    className="px-4 py-2 bg-[#D4AF37] hover:bg-[#C5A030] text-black rounded-lg font-medium transition-colors flex items-center gap-2 text-sm"
                  >
            ...
            <div className="mb-6 rounded-lg border border-border/70 bg-muted/20 px-3 py-2.5">
            ...
            <div className="space-y-8 mt-2">
              {Object.keys(groupedByFloor).length > 0 ? (
                Object.entries(groupedByFloor).map(([floor, floorRooms]) => (
                  <div key={floor}>
                    <div className="flex items-center gap-3 mb-4 px-1">
                      <h3 className="text-[12px] font-semibold text-muted-foreground uppercase tracking-wider">{floor}</h3>
                      <div className="flex-1 h-px bg-border/60" />
                      <span className="text-[12px] text-muted-foreground/80">{floorRooms.length} rooms</span>
                    </div>
                      ...
                      {floorRooms.map((room) => {
                        ...
                        return (
                          <button
                            ...
                            className={`group relative bg-white dark:bg-card border border-border/80 rounded-xl p-4 hover:border-primary/50 hover:shadow-md transition-all duration-200 text-center flex flex-col items-center justify-center w-[118px] h-[116px] ${!can("manageRooms") ? 'cursor-default' : 'cursor-pointer'}`}
                          ...
                          >
                            ...
                            <Bed className="w-5 h-5 text-slate-400/80 mb-1 group-hover:text-primary transition-colors" />
                            <span className="text-[18px] font-bold text-slate-800 dark:text-foreground tracking-tight leading-none mb-1.5">{room.roomNumber}</span>
                            
                            {roomLevelMaintenance ? (
                              <span className="text-xs text-slate-500 font-medium">Maintenance</span>
                            ) : (
                              <span className="text-xs text-slate-500 font-medium">
                                {room.currentOccupancy || 0}/{effectiveCapacity}
                                {bedsInMaintenance > 0 && (
                                  <span className="ml-[2px]">({bedsInMaintenance}M)</span>
                                )}
                              </span>
                            )}
                          </button>
                ...
                <div className="text-center py-16 bg-card border border-border rounded-lg border-dashed">
                  ...
                  <p className="text-sm font-medium text-foreground">No rooms found matching your filters</p>
                  <p className="text-xs text-muted-foreground mt-1">Try adjusting your search or add a new room.</p>
                </div>
              )}
              
              {floorKeys.length > FLOORS_PER_PAGE && (
                <div className="flex items-center justify-between pt-4 border-t border-border mt-6 px-1">
                  <span className="text-xs text-muted-foreground">
                    Showing floors {(currentPage - 1) * FLOORS_PER_PAGE + 1} to {Math.min(currentPage * FLOORS_PER_PAGE, floorKeys.length)} of {floorKeys.length}
                  </span>
                  <div className="flex gap-1">
                    <button
                      disabled={currentPage === 1}
                      onClick={() => setCurrentPage(p => p - 1)}
                      className="px-3 py-1 text-xs border border-border rounded hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed text-foreground"
                    >
                      Previous
                    </button>
                    <button
                      disabled={currentPage >= Math.ceil(floorKeys.length / FLOORS_PER_PAGE)}
                      onClick={() => setCurrentPage(p => p + 1)}
                      className="px-3 py-1 text-xs border border-border rounded hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed text-foreground"
                    >
                      Next
      ...
      {activeTab === "forecast" && (
        <div className="space-y-6">
          ...
          />
          {forecastLoading ? (
            <div className="forecast-empty-state bg-card border border-border rounded-lg p-8">
              <Clock3 size={28} />
