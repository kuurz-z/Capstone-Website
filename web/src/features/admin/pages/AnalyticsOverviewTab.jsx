    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <ReportChartPanel
          ...
          actions={
            <Link to="/admin/reservations" className="text-sm font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1.5 transition-colors">
              Open reservations
          ...
          <div className="flex flex-col gap-3 py-2">
            {inquiries.length > 0 ? (
              inquiries.slice(0, 5).map((item) => (
                <div key={item.id} className="relative bg-slate-50 border border-slate-100 rounded-xl p-4 flex flex-col gap-1.5 hover:bg-slate-100 transition-colors group cursor-pointer">
                  <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                    <MessageSquare size={13} className="text-slate-400 group-hover:text-blue-500 transition-colors" /> {item.name || "Unknown"}
                  </span>
                  <div className="text-sm font-semibold text-slate-800">{item.email || "-"}</div>
                  <p className="flex items-center gap-1.5 text-xs text-slate-500 mt-1">
                    <span className="bg-slate-200/50 px-2 py-0.5 rounded-md">{formatBranch(item.branch)}</span> 
                    <span className="text-slate-300">•</span>
                    <Clock size={12} className="text-slate-400" /> {item.createdAt ? formatDateTime(item.createdAt) : "-"}
                  </p>
                </div>
              ))
            ) : (
          ...
          <div className="flex flex-col gap-3 py-2">
            {reservations.length > 0 ? (
              reservations.slice(0, 5).map((item) => (
                <div key={item.id} className="relative bg-slate-50 border border-slate-100 rounded-xl p-4 flex justify-between items-start gap-4 hover:bg-slate-100 transition-colors group cursor-pointer">
                  <div className="flex flex-col gap-1.5">
                    <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                      <CalendarCheck size={13} className="text-slate-400 group-hover:text-blue-500 transition-colors" /> {item.guestName || "Unknown"}
                    </span>
                    <div className="text-sm font-semibold text-slate-800">{item.roomType || "-"}</div>
                    <p className="flex items-center gap-1 text-xs text-slate-500 mt-1">
                      <span className="bg-slate-200/50 px-2 py-0.5 rounded-md">{formatBranch(item.branch)}</span>
                      <span className="text-slate-300 mx-1">•</span>
                      Move in: <span className="font-medium text-slate-700">{formatDate(item.moveInDate || item.createdAt)}</span>
                    </p>
                  </div>
