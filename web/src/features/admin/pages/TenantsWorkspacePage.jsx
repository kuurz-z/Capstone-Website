...
const RowActionsMenu = ({ row, onSelect, onAction }) => {
 ...
 return (
 <div className="flex items-center justify-center gap-2" data-action-cell="true">
 <button
 type="button"
 ...
 <div
 ref={menuRef}
 className="fixed z-[60] w-48 overflow-hidden rounded-lg border border-border bg-popover shadow-lg"
 data-action-portal="true"
 ...
 className === "tenant-dropdown-item--danger" ? "text-red-600" : "text-foreground"
 }`}
...
export default function TenantsWorkspacePage() {
 ...
 const startRecord = sortedTenants.length === 0 ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1;
 const endRecord = Math.min(currentPage * ITEMS_PER_PAGE, sortedTenants.length);

 const selectedTenantRow = useMemo(
 () => sortedTenants.find((tenant) => tenant.reservationId === selectedReservationId) || null,
 ...
 const extensionHistory = (leaseInfo.extensionHistory || []).map((entry) => ({
 id: entry.id,
 ...
 id: entry.id,
 branch: formatBranch(entry.branch || basicInfo.branch || base.branch || "") || "N/A",
 ...
 .join("")
 .toUpperCase() || "?",
 name: basicInfo.name || base.tenantName || (tenantDetailLoading ? "Loading..." : "Tenant"),
 ...
 daysRemaining: leaseInfo.daysUntilLeaseEnd ?? base.daysUntilLeaseEnd ?? null,
 ...
 }, [selectedReservationId, selectedTenantRow, tenantDetail, tenantDetailLoading]);
 ...
 <div>
 <h2 className="mb-1 text-2xl font-semibold text-foreground">Tenants</h2>
 ...
 <div>
 <div className="text-2xl font-semibold text-blue-600">{summaryItems[0].value}</div>
 <div className="text-xs text-muted-foreground uppercase tracking-wide">Total Residents</div>
 </div>
 </div>
 </div>
 <div className="bg-card border border-border rounded-lg p-4">
 ...
 <div>
 <div className="text-2xl font-semibold text-green-600">{summaryItems[1].value}</div>
 <div className="text-xs text-muted-foreground uppercase tracking-wide">Active Tenants</div>
 </div>
 </div>
 </div>
 <div className="bg-card border border-border rounded-lg p-4">
 ...
 <div>
 <div className="text-2xl font-semibold text-amber-500">{summaryItems[2].value}</div>
 <div className="text-xs text-muted-foreground uppercase tracking-wide">Expiring Soon</div>
 </div>
 </div>
 </div>
 <div className="bg-card border border-border rounded-lg p-4">
 ...
 <div>
 <div className="text-2xl font-semibold text-red-500">{summaryItems[3].value}</div>
 <div className="text-xs text-muted-foreground uppercase tracking-wide">Overdue Payments</div>
 </div>
 </div>
 </div>
 </div>
 ...
 <button className="px-4 py-2 border border-border rounded-md hover:bg-muted transition-colors flex items-center gap-2 text-sm">
 ...
 active
 ? "bg-border text-card-foreground"
 ...
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Tenant</th>
 {isOwner ? (
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Branch</th>
 ) : null}
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Room + Bed</th>
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Contract End</th>
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Billing</th>
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Contract</th>
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Occupancy</th>
 <th className="text-left py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Next Action</th>
 <th className="text-center py-3 px-4 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Actions</th>
 </tr>
 </thead>
 <tbody>
 {paginatedTenants.map((tenant) => (
 <tr key={tenant.reservationId || tenant.tenantName} className="border-b border-border hover:bg-muted/50 transition-colors">
 ...
 <div>
 <div className="font-medium text-sm text-foreground">{tenant.tenantName}</div>
 <div className="text-xs text-muted-foreground">{tenant.contact?.email || "No email"}</div>
 <div className="text-xs text-muted-foreground">{tenant.contact?.phone || "No phone"}</div>
 </div>
 </div>
 </td>
 {isOwner ? <td className="py-3 px-4 text-sm text-foreground">{formatBranch(tenant.branch) || "—"}</td> : null}
 <td className="py-3 px-4">
 <div className="text-sm font-medium text-foreground">{tenant.room || "—"}</div>
 <div className="text-xs text-muted-foreground">{tenant.bed || "No bed"}</div>
 </td>
 <td className="py-3 px-4">
 <div className="text-sm text-foreground">{fmtDate(tenant.leaseEndDate)}</div>
 ...
 <td className="py-3 px-4"><StatusBadge status={tenant.paymentStatus} /></td>
 <td className="py-3 px-4"><StatusBadge status={tenant.leaseStatus} /></td>
 <td className="py-3 px-4"><StatusBadge status={tenant.stayStatus} /></td>
 <td className="py-3 px-4 text-sm text-muted-foreground">{tenant.nextActionLabel}</td>
 <td className="py-3 px-4">
 <RowActionsMenu row={tenant} onSelect={setSelectedReservationId} onAction={setActionState} />
 </td>
 </tr>
 ))}
 </tbody>
