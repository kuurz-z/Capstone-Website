...
export default function StayExtensionRequests({ onReviewed }) {
  const [statusFilter, setStatusFilter] = useState('pending'), [pendingCount, setPendingCount] = useState(0);
  const [requests, setRequests] = useState([]), [error, setError] = useState('');
  const [notes, setNotes] = useState({}), [busy, setBusy] = useState(null);
  const load = useCallback(async () => {
    try { const result = await authFetch(`/tenant/stay-extension-requests?status=${statusFilter}`); setRequests(result.requests || []); setPendingCount(result.pendingCount || 0); setError(''); }
    catch (err) { setError(err.message || 'Unable to load extension requests.'); }
  }, [statusFilter]);
  useEffect(() => { load(); const refresh = () => load(); window.addEventListener('focus', refresh); const timer = setInterval(refresh, 30000); return () => { window.removeEventListener('focus', refresh); clearInterval(timer); }; }, [load]);
  ...
  return <section className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900" aria-label="Stay extension requests">
    <div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-gray-900 dark:text-white">Stay extension requests ({pendingCount} pending)</h2><button type="button" onClick={load} className="text-sm text-blue-700 dark:text-blue-300">Refresh</button></div>
    <label className="mt-3 block text-sm">Status <select aria-label="Extension request status" value={statusFilter} onChange={event => setStatusFilter(event.target.value)} className="rounded border bg-white p-2 text-gray-900"><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option><option value="all">All</option></select></label>
    ...
    <div className="space-y-4">{requests.map((request) => <article key={request._id} className="mt-4 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
      ...
      <dl className="mt-2 grid gap-2 text-sm text-gray-700 dark:text-gray-200 sm:grid-cols-2">
        <div><dt className="font-medium">Submitted</dt><dd>{date(request.createdAt)}</dd></div><div><dt className="font-medium">Review</dt><dd>{request.acknowledgedAt ? `Reviewed ${date(request.acknowledgedAt)}` : 'Awaiting review'}</dd></div>{request.fulfillmentState ? <div><dt className="font-medium">Fulfillment</dt><dd>{request.fulfillmentState.replaceAll('_', ' ')}</dd></div> : null}<div><dt className="font-medium">Current contract</dt><dd>{date(request.currentStartDate)} – {date(request.currentEndDate)}</dd></div>
      ...
      {request.status === 'pending' ? <div className="mt-3 space-y-3">
        ...
        <div className="flex gap-3">{!request.acknowledgedAt ? <button type="button" disabled={Boolean(busy)} onClick={() => review(request, 'acknowledged')} className="rounded border px-4 py-2 text-sm">Acknowledge</button> : null}<button type="button" disabled={Boolean(busy)} onClick={() => review(request, 'approved')} className="rounded bg-blue-900 px-4 py-2 text-sm text-white disabled:opacity-50">Approve extension</button><button type="button" disabled={Boolean(busy)} onClick={() => review(request, 'rejected')} className="rounded border border-red-700 px-4 py-2 text-sm text-red-700 disabled:opacity-50 dark:text-red-300">Reject</button></div>
