...
import { createStayExtension, getMyStayExtension, reviewStayExtension, serializeStayExtension } from '../services/stayExtensionRequestService.js';
...
adminStayExtensionRoutes.get('/stay-extension-requests', ...guards, wrap(async (req, res) => {
  ...
  const selected = ['pending', 'approved', 'rejected'].includes(req.query.status) ? { ...filter, status: req.query.status } : filter;
  const pendingCount = await StayExtensionRequest.countDocuments({ ...filter, status: 'pending' });
  const requests = await StayExtensionRequest.find(selected).sort({ createdAt: -1 }).limit(100)
    .populate('tenantId', 'firstName lastName email').populate('roomId', 'name roomNumber').lean();
  res.json({ requests: await Promise.all(requests.map(serializeStayExtension)), pendingCount });
}));
